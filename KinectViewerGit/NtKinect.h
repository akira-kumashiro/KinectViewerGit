...
/*	cv::Mat bufferDepth;
	cv::Mat depthImage;
	void setDepth(bool raw = true) { setDepth(bufferDepth, raw); }
	void setDepth(cv::Mat& bufferDepth, bool raw = true) {
		updateDepthFrame();
		bufferDepth = cv::Mat(depthHeight, depthWidth, CV_16UC1);
		depthImage = cv::Mat(depthHeight, depthWidth, CV_8UC1);
		if (!raw)
		{
			bufferDepth.convertTo(depthImage, CV_8U, -255.0f / 8000.0f, 255.0f);
		}
	}*/
	cv::Mat depthImage;
	...
	cv::Mat depthMat;
	void setDepth(bool raw = true) { setDepth(depthImage,rawDepthImage, raw); }
	void setDepth(cv::Mat& depthImage,cv::Mat& rawDepthImage, bool raw = true) {
		updateDepthFrame();
		depthImage = cv::Mat(depthHeight, depthWidth, CV_16UC1);
		rawDepthImage = cv::Mat(depthHeight, depthWidth, CV_16U);
		depthMat = cv::Mat(depthHeight, depthWidth, CV_8UC1);
		for (int i = 0; i < depthImage.total(); i++) 
		{
			double rate = depthBuffer[i] / (double)maxDepth;
			depthImage.at<UINT16>(i) = (raw) ? depthBuffer[i] : (UINT16)(255 * (1 - rate)); // [0, 65535]
			rawDepthImage.at<UINT16>(i) = depthBuffer[i];
		}
		depthImage.convertTo(depthMat, CV_8U);
		rawDepthImage.convertTo(rawDepthImage, CV_16U);
		//depthImage.convertTo(depthMat, CV_8UC1);
		//depthMat.convertTo(depthImage, CV_8U, 2);
	}

	/*void setDepth(cv::Mat depthImage,cv::Mat rawDepthImage, bool raw) {
		updateDepthFrame();
		depthImage = cv::Mat(depthHeight, depthWidth, CV_16UC1);
